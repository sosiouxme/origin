...
func (jt jsonTime) Sub(sub jsonTime) jsonDuration {
	return jsonDuration(time.Time(jt).Sub(time.Time(sub)))
}
...
func (d *AppCreate) Check() types.DiagnosticResult {
	...
	go func() {
		...
		if !d.prepareForApp() || !d.createAndTestAppDC() || !d.createAndTestService() {
...
func (d *AppCreate) testHttp(url string, timeout int64, retry int) error {
