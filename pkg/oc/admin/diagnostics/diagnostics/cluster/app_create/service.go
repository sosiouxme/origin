...
func (d *AppCreate) createAndTestService() bool {
	result := &d.result.Service
	result.BeginTime = jsonTime(time.Now())
	defer recordTrial(result)
	if !d.createService() || !d.testServiceEndpoint() {
		return false
	}
	if d.skipServiceConnect {
		...
		result.Success = d.testServiceConnect()
	}
...
func (d *AppCreate) testServiceEndpoint() bool {
...
func (d *AppCreate) testServiceConnect() bool {
